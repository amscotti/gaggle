...
//!              + language-detected config.toml / gitignore snippet
//! verify.rs    run verify commands (cargo test etc.) after a fix
...
//! → committing → done (failed = quarantine). The loop is resumable from
//! `.review/state.json` at any point.
//!
